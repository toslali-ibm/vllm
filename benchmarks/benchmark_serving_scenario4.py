...
async def benchmark(
    ...
    if isinstance(metrics, BenchmarkMetrics):
        result = {
            ...
            "prompt_latencies": [output.latency for output in outputs],
        }
        result["prompts"] = []
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not args.save_detailed:
        if "prompts" in result_json:
            del result_json["prompts"]
        if "prompts" in benchmark_result:
            del benchmark_result["prompts"]
...
if __name__=="__main__":
    parser = FlexibleArgumentParser(description="Benchmark the serving for scenario4.")
    add_cli_args(parser)
